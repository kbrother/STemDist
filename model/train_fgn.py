...
# python -m model.train_fgn -de 0 -d ../data/METR-LA -lr 1e-2 -e 100
if __name__ == "__main__":
    ...
    model = FGN(pre_length=12, embed_size=args.embed_size, feature_size=num_nodes, seq_length=12, hidden_size=args.hidden_size)
    ...
    for i in range(args.epochs):
        ...
        for iter, (x, y) in enumerate(tqdm(dataloader['train_loader'].get_iterator())):
            trainx = torch.tensor(x, device=device, dtype=torch.float)        # B, L, N
            trainx = trainx[:,:,:,0]
            trainy = torch.tensor(y, device=device, dtype=torch.float)
            trainy = trainy[:,:,:,0].transpose(2, 1)   # B, N, l

            output = model(trainx)   # B, N, L        
