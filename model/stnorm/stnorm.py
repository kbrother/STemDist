...
class TNorm(nn.Module):
    def __init__(self,  channels, track_running_stats=True, momentum=0.1):
    ...
    def forward(self, x):
        if self.track_running_stats:
            ...
            var = x.var((0, 2, 3), keepdims=True, unbiased=False)
            if self.training:
                n = x.shape[3] * x.shape[0] * x.shape[2]
                with torch.no_grad():
...
class STNorm(nn.Module):
    def __init__(self, device, tnorm_bool=False, snorm_bool=False, in_dim=1,out_dim=12, channels=16,kernel_size=2,blocks=4,layers=2):
    ...
    # batch size x len x num node x feat
