...
class GwaveTrajFreq:
    ...
    def __init__(self, data, args, device):
        ...
        print(f'feat x shape: {self.synx.shape}')  # [23,12,207,2]
        print(f'feat y shape: {self.syny.shape}')  # [23,12,207]
    ...
    def train(self):
        ...
        syn_f = syn_f[:,:,:, :synx.shape[-1]]
        real_f = torch.fft.rfft(torch.tensor(data['train_loader'].xs), dim=-1)
        real_f = torch.view_as_real(real_f).reshape(real_f.shape[0], real_f.shape[1], real_f.shape[2], -1)
        real_f = real_f[:,:,:, :torch.tensor(data['train_loader'].xs).shape[-1]]
        ...
        for i in tqdm(range(args.epochs)):
            ...
            # compute loss for freq domain
            output_syn_f = student_net(syn_f.transpose(1, 3), flat_param=student_params[-1]).squeeze()
            output_real_f = student_net(real_f.transpose(1, 3), flat_param = torch.tensor(expert_traj[-1], dtype=torch.float32).view(-1)).squeeze()
            # print(output_syn_f.shape)  # [23,12,207]
            print(output_real_f.shape)
