...
class condTSC:
    ...
    def train(self):
        ...
        for i in tqdm(range(args.epochs)):
            ...
            student_net = GLinear(args, num_nodes, in_dim).to(self.device)                  
            ...
            for multi_step in range(1):                
                ...
                for step in range(args.syn_steps):
                    output_syn = student_net(synx, flat_param=student_params)
                    loss_syn = F.mse_loss(output_syn, syny)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--params', type=str, default='../data/params/METR-LA-glinear/')
    parser.add_argument('-sp', '--save_path', type=str, default='results/')
    parser.add_argument('-ne', '--num_experts', type=int, default=40)
    parser.add_argument('-mse', '--max_start_epoch', type=int, default=17, help='max epoch we can start at')
